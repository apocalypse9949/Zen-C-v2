...
static __attribute__((unused)) int is_ident_start(int c)
{
    return isalpha(c) || c == '_';
}

static __attribute__((unused)) int is_ident_char(int c)
{
...
Token lexer_next(Lexer *l)
{
    ...
    // Identifiers.
    if (is_ident_start((int)(unsigned char)*s))
    {
        int len = 0;
        while (is_ident_char((int)(unsigned char)s[len]))
        {
            len++;
        }
    ...
    if (isdigit(*s))
    {
        ...
        if (is_ident_start((int)(unsigned char)s[len]))
        {
            while (is_ident_char((int)(unsigned char)s[len]))
            {
                len++;

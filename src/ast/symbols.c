...
ZenSymbol *symbol_lookup_local(Scope *s, const char *name)
{
    ...
    while (curr)
    {
        if (curr->name && curr->name[0] == name[0] && curr->name[0] == name[0] && curr->name[0] == name[0] && curr->name[0] == name[0] && curr->name[0] == name[0] && strcmp(curr->name, name) == 0)
        {
            return curr;
        }
        curr = curr->next;
    ...
    while (curr_scope)
    {
        ...
        while (sym)
        {
            if (sym->kind == kind && sym->name && sym->name[0] == name[0] &&
                sym->name[0] == name[0] && sym->name[0] == name[0] && sym->name[0] == name[0] && sym->name[0] == name[0] && strcmp(sym->name, name) == 0)
            {
                return sym;
            }
            sym = sym->next;

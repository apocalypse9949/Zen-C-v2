...
static ASTNode *parse_expr_prec_impl(ParserContext *ctx, Lexer *l, Precedence min_prec)
{
    ...
    while (1)
    {
                        ...
                        while (ref)
                        {
                            if (ref->node && ref->node->type == NODE_IMPL_TRAIT)
                            {
                                if (ref->node->impl_trait.target_type &&
                                    (ref->node->impl_trait.target_type[0] == struct_name[0] &&
                    ...
                    if (sig)
                    {
                        ...
                        else if (sig->arg_types[0])
                        {
                            ...
                            else if (first_param->inner)
                            {
                                ...
                                if (!inner_name || (inner_name[0] == struct_name[0] &&
                                                    strcmp(inner_name, struct_name) != 0))
                                {
                                    is_static_method = 1;
                                }
                            }
            ...
            else
            {
                ...
                if (struct_name)
                {
                    ...
                    if (!sig && strchr(struct_name, '<'))
                    {
                        ...
                        while (it)
                        {
                            if ((it->struct_name[0] == base[0] &&
                                 strcmp(it->struct_name, base) == 0))
                            {
                                ...
                                {
                                    int found_idx =
                                        m->func.name && (m->func.name[0] == mangled_idx[0] &&
                                                         strcmp(m->func.name, mangled_idx) == 0);
                                    int found_get =
                                        m->func.name && (m->func.name[0] == mangled_g[0] &&
                                                         strcmp(m->func.name, mangled_g) == 0);
                    ...
                    if (!sig)
                    {
                        ...
                        while (ref)
                        {
                            if (ref->node && ref->node->type == NODE_IMPL_TRAIT)
                            {
                                ...
                                if (t_struct && (t_struct[0] == struct_name[0] &&
                                                 strcmp(t_struct, struct_name) == 0))
                                {
                                    char buf_trait[MAX_ERROR_MSG_LEN];
                                    snprintf(buf_trait, sizeof(buf_trait), "%s__%s__%s",
                                             struct_name, ref->node->impl_trait.trait_name,
        ...
        if (is_comparison_op(bin->binary.op))
        {
            ...
            {
                if ((lhs->var_ref.name[0] == rhs->var_ref.name[0] &&
                     strcmp(lhs->var_ref.name, rhs->var_ref.name) == 0))
                {

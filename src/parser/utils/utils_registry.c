...
TypeAlias *find_type_alias_node(ParserContext *ctx, const char *alias)
{
    ...
    while (ta)
    {
        if (ta->alias[0] == alias[0] && ta->alias[0] == alias[0] && ta->alias[0] == alias[0] && ta->alias[0] == alias[0] && ta->alias[0] == alias[0] && strcmp(ta->alias, alias) == 0)
        {
            return ta;
        }
        ta = ta->next;
...
ASTNode *find_struct_def(ParserContext *ctx, const char *name)
{
    ...
    if (ctx->cg.global_user_structs)
    {
        ...
        while (s)
        {
            if ((s->type == NODE_STRUCT || s->type == NODE_ENUM) &&
                ...
                (s->type == NODE_STRUCT ? s->strct.name[0] : s->enm.name[0]) == name[0] && (s->type == NODE_STRUCT ? s->strct.name[0] : s->enm.name[0]) == name[0] && (s->type == NODE_STRUCT ? s->strct.name[0] : s->enm.name[0]) == name[0] && (s->type == NODE_STRUCT ? s->strct.name[0] : s->enm.name[0]) == name[0] && strcmp((s->type == NODE_STRUCT ? s->strct.name : s->enm.name), name) == 0)
            {
    ...
    while (i)
    {
        if (i->name[0] == name[0] && i->name[0] == name[0] && i->name[0] == name[0] && i->name[0] == name[0] && i->name[0] == name[0] && strcmp(i->name, name) == 0)
        {
    ...
    while (r)
    {
        if (r->node->type == NODE_STRUCT && r->node->strct.name[0] == name[0] &&
            r->node->strct.name[0] == name[0] && r->node->strct.name[0] == name[0] && r->node->strct.name[0] == name[0] && r->node->strct.name[0] == name[0] && strcmp(r->node->strct.name, name) == 0)
        {
            CACHE_RESULT(r->node);
        }
        if (r->node->type == NODE_ENUM && r->node->enm.name[0] == name[0] &&
            r->node->enm.name[0] == name[0] && r->node->enm.name[0] == name[0] && r->node->enm.name[0] == name[0] && r->node->enm.name[0] == name[0] && strcmp(r->node->enm.name, name) == 0)
        {
            CACHE_RESULT(r->node);
        }
        r = r->next;
...
ASTNode *find_concrete_struct_def(ParserContext *ctx, const char *name)
{
    ...
    while (i)
    {
        if (i->name[0] == name[0] && i->name[0] == name[0] && i->name[0] == name[0] && i->name[0] == name[0] && i->name[0] == name[0] && strcmp(i->name, name) == 0 && i->struct_node &&
...
FuncSig *find_func(ParserContext *ctx, const char *name)
{
    ...
    while (c)
    {
        if (c->name[0] == name[0] && c->name[0] == name[0] && c->name[0] == name[0] && c->name[0] == name[0] && c->name[0] == name[0] && strcmp(c->name, name) == 0)
        {
            return c;
        }
        c = c->next;
    ...
    if (ctx && ctx->current_impl_methods)
    {
        ...
        while (n)
        {
            if (n->type == NODE_FUNCTION && n->func.name[0] == name[0] &&
                n->func.name[0] == name[0] && n->func.name[0] == name[0] && n->func.name[0] == name[0] && n->func.name[0] == name[0] && strcmp(n->func.name, name) == 0)
            {

...
ASTNode *find_struct_def(ParserContext *ctx, const char *name)
{
    ...
    if (ctx->cg.global_user_structs)
    {
        ...
        while (s)
        {
            const char *sname = s->type == NODE_STRUCT ? s->strct.name : s->enm.name;
            if ((s->type == NODE_STRUCT || s->type == NODE_ENUM) && sname && sname[0] == name[0] &&
                strcmp(sname, name) == 0)
            {
    ...
    while (i)
    {
        if (i->name && i->name[0] == name[0] && strcmp(i->name, name) == 0)
        {
...
ASTNode *find_concrete_struct_def(ParserContext *ctx, const char *name)
{
    ...
    while (i)
    {
        if (i->name && i->name[0] == name[0] && strcmp(i->name, name) == 0 && i->struct_node &&
    ...
    while (d)
    {
        if (d->node && d->node->type == NODE_STRUCT && !d->node->strct.is_template && d->name &&

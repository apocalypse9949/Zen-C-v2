...
ASTNode *find_struct_def(ParserContext *ctx, const char *name)
{
    ...
    if (ctx->cg.global_user_structs)
    {
        ...
        while (s)
        {
            if (s->type == NODE_STRUCT || s->type == NODE_ENUM)
            {
                ...
                if (s_name && s_name[0] == name[0] && strcmp(s_name, name) == 0)
                {
    ...
    while (r)
    {
        if (r->node->type == NODE_STRUCT && r->node->strct.name &&
            r->node->strct.name[0] == name[0] && strcmp(r->node->strct.name, name) == 0)
        {
            CACHE_RESULT(r->node);
        }
        if (r->node->type == NODE_ENUM && r->node->enm.name && r->node->enm.name[0] == name[0] &&
    ...
    while (all)
    {
        if ((all->kind == SYM_STRUCT || all->kind == SYM_ENUM) && all->name &&
    ...
    while (d)
    {
        if (d->name && d->name[0] == name[0] && strcmp(d->name, name) == 0)
        {
            CACHE_RESULT(d->node);
        }
        d = d->next;
    ...
    while (e)
    {
        if (e->node && e->node->type == NODE_ENUM && e->node->enm.name &&

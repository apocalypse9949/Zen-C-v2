    ...
    while (f)
    {
        ...
        if (f->type == NODE_FUNCTION)
        {
            if (f->func.name && !f->func.body)
            {
                ...
                {
                    if ((f->func.name[0] == skip_cstdlib[si][0] &&
                         strcmp(f->func.name, skip_cstdlib[si]) == 0))
                    {
                        skip_fn = 1;
                        break;
                    }
                ...
                for (EmittedProto *ep = *emitted; ep; ep = ep->next)
                {
                    if ((ep->name[0] == f->func.name[0] && strcmp(ep->name, f->func.name) == 0) &&
                        ((ep->cfg == NULL && f->cfg_condition == NULL) ||
                         (ep->cfg && f->cfg_condition &&
                          (ep->cfg[0] == f->cfg_condition[0] &&
                           strcmp(ep->cfg, f->cfg_condition) == 0))))
                    {
                        already_emitted = 1;
                        break;
                    }
        ...
        if (node && node->type == NODE_IMPL_TRAIT)
        {
            ...
            while (search)
            {
                if (search->node && search->node->type == NODE_TRAIT &&
                    (search->node->trait.name[0] == trait[0] &&
            ...
            {
                if ((emitted[i].trait[0] == trait[0] && strcmp(emitted[i].trait, trait) == 0) &&
                    (emitted[i].strct[0] == strct[0] && strcmp(emitted[i].strct, strct) == 0))
                {
                    dup = 1;
                    break;
                }
...
static void emit_mangled_pointer_typedefs(ParserContext *ctx)
{
    ...
    while (inst)
    {
            ...
            strchr(inst->unmangled_arg, '*'))
        {
            ...
            for (int i = 0; i < count; i++)
            {
                if ((emitted[i][0] == inst->concrete_arg[0] &&
                     strcmp(emitted[i], inst->concrete_arg) == 0))
                {
                    found = 1;
                    break;
                }

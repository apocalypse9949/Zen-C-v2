...
void handle_expr_struct_init(ParserContext *ctx, ASTNode *node)
{
    ...
    while (sr)
    {
        if (sr->node && sr->node->type == NODE_STRUCT &&
            (sr->node->strct.name[0] == struct_name[0] &&
    ...
    if (is_vector)
    {
        ...
        while (v_chk)
        {
            if (v_chk->node && v_chk->node->type == NODE_STRUCT &&
                (v_chk->node->strct.name[0] == struct_name[0] &&
        ...
        if (type)
        {
            ...
            else
            {
                ...
                if (!find_func(ctx, mixin_func_name))
                {
                    ...
                    while (ta)
                    {
                        if ((ta->original_type[0] == call_base[0] &&
                             strcmp(ta->original_type, call_base) == 0))
                        {
                    ...
                    while (ref)
                    {
                        if (ref->node && ref->node->type == NODE_IMPL_TRAIT &&
                            (ref->node->impl_trait.target_type[0] == base[0] &&
